...
public class CalculatorTest {
    ...
    @Test void worksForRandomInputs() {
        ...
        assertTrue(s.matches("(?s).*\\+\\s*=\\s*"+(a+b)+".*"), "Addition result missing/incorrect");
        assertTrue(s.matches("(?s).*\\-\\s*=\\s*"+(a-b)+".*"), "Subtraction result missing/incorrect");
        assertTrue(s.matches("(?s).*\\*\\s*=\\s*"+(a*b)+".*"), "Multiplication result missing/incorrect");
        assertTrue(s.matches("(?s).*/\\s*=\\s*"+(a/b)+".*"), "Division result missing/incorrect");
    }
}
